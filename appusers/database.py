...
class Group:
    ...
    def get_list(cls, filters):
        ...
        # This version supports only 'groupname' key in 'filters'
        if 'groupname' in filters:
            groupnames = filters['groupname'].split(',')
            groups = [g for g in cls._db.values() if g.groupname in groupnames]
        else:
            groups = list(cls._db.values())
...
class User:
    ...
    def get_list(cls, filters):
        ...
        # This version supports only 'username' key in 'filters'
        if 'username' in filters:
            usernames = filters['username'].split(',')
            users = [u for u in cls._db.values() if u.username in usernames]
        else:
            users = list(cls._db.values())
